class Api::V1::ItemsController < ApplicationController
  ...
  def update
    distributor = get_distributor(api_key_param[:api_key])
    item = Item.update(item_id_param[:id], name: item_params[:name], alc_type: item_params[:alc_type], alc_category: item_params[:alc_category], price: item_params[:price], quantity: item_params[:quantity], ounces: item_params[:ounces], unit: item_params[:unit], thumbnail: item_params[:thumbnail], distributor_id: distributor.id)
